...
def read_csv(file_name: str) -> pd.DataFrame:
    ...
    SelfLogger.get_inst().get_logger(__name__).info("CSV file  read.")
    return table


def read_pickle(file_name: str) -> pd.DataFrame:
    table = pd.read_pickle(file_name)
    SelfLogger.get_inst().get_logger(__name__).info("Dataframe read from a pickle.")
    return table


def write_pickle(df: pd.DataFrame, file_name: str) -> None:
    df.to_pickle(f"./result/{file_name}")
    SelfLogger.get_inst().get_logger(__name__).info("Dataframe saved as a pickle.")
